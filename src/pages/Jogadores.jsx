...
function Jogadores() {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Tem certeza que deseja excluir este jogador?')) {
        const jogadorToDelete = jogadores.find(j => j.id === id);
        setJogadores(jogadores.filter(j => j.id !== id)); // Remove localmente para refresh imediato
        try {
          await api.delete(`/jogador/${id}`);
        } catch (error) {
          console.error("Erro ao deletar jogador:", error);
          setJogadores([...jogadores, jogadorToDelete]); // Adiciona de volta se falhar
          if (error.response && error.response.status === 401) {
            handleLogout(); // Desloga se o token for inválido
          }
        }
